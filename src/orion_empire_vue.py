...
class Vue():
    ...
    def afficherinitpartie(self,mod):
        ...
        self.voirsysteme(mod.joueurs[self.nom].systemeorigine) #Commencer en Vue SystemeOrigine si PlaneteOrigine pas disponible
    ...
    def afficherBatiment(self,Batiment):
        ...
        for i in self.modes["planetes"].keys():
                if i == Batiment.planeteid:
                    ...
                    self.modes["planetes"][i].canevas.create_image(Batiment.x,Batiment.y, image=im,tags=(Batiment.id,Batiment.type))
                    
                    self.modes["planetes"][i].minimap.create_oval(x*t - p, y*t - p, x*t + p, y*t + p,  fill = couleur,tags=(Batiment.id,Batiment.type))
...
class Perspective(Frame):
    def __init__(self,parent):
        ...
        rWidth = 9
        rHeight = 2
        self.ressources=Label(self.cadreRessources,width=rWidth,height=rHeight,bg="yellow",textvariable=self.r1) #!
        self.ressources.pack(side=LEFT)
        self.ressources=Label(self.cadreRessources,width=rWidth,height=rHeight,bg="green",textvariable=self.r2) #!
        self.ressources.pack(side=LEFT)
...
class VueGalaxie(Perspective):
    def __init__(self,parent):
        ...
        self.btncreervaisseau=Button(self.cadreetataction,text="Creer Vaisseau",command=self.creervaisseauGalactique)
        self.btncreervaisseau.pack()
        
        self.btncreerstationGalactique=Button(self.cadreetataction,text="Creer Station Galactique",command=self.creerstationGalactique)##############################  MODIF TRISTAN
        ...
        self.btndechargervaisseau=Button(self.cadreetatmsg,text="Décharger vaisseau",command=self.dechargerVaisseauGalactique)
        self.btndechargervaisseau.pack(side=BOTTOM)
        
        self.btncreervaisseau=Button(self.cadreetatmsg,text="Upgrade vitesse vaisseau",command=self.upgradeVitesseVaisseau)
        self.btncreervaisseau.pack()
   ...
   #! --------------------------------------------- DeBUT MODIF       
    ...
    def afficherpartie(self,mod):
        ...
        for k in mod.joueurscles:
            ...
            for j in i.vaisseauxinterstellaires:
                ...
                self.canevas.create_image(jx,jy,image = self.img[k][index],tags=(j.proprietaire,"vaisseauinterstellaire",j.id,"artefact"))
                index += 1
                ...
                if i.nom == self.parent.nom:
                    self.minimap.create_rectangle((j.x*me)-m,(j.y*me)-m,(j.x*me)+m,(j.y*me)+m,fill=i.couleur,
        ...
        for i in mod.joueurscles:################################################## MODIF TRISTAN
            ...
            for j in i.stationGalactiques:
                self.canevas.create_oval(j.x*e-5,j.y*e-5,j.x*e-15,j.y*e-15, fill=i.couleur,outline="white",tags=(j.proprietaire,"StationGalactique",j.id,"artefact"))
    #! -------------------------------------------------- D�BUT MODIF    
    def changeetatsystem(self,nom,systeme): 
        ...
        self.minimap.create_oval((systeme.x*me)-m,(systeme.y*me)-m,(systeme.x*me)+m,(systeme.y*me)+m,fill=systeme.etoile.type,tags=("systeme",systeme.id))
    #! ------------------------------------------------------- FIN MODIF
    ...
    def afficherselection(self):
        ...
        if self.maselection!=None:
            ...
            elif self.maselection[1]=="StationGalactique": ################################################################## MODIF TRISTAN
    ...
    def cliquervue(self,evt):
        ...
        if t and t[0]!="current":
            ...
            elif t[1] =="StationGalactique": ############################################################ MODIF TRISTAN
                self.maselection=[self.parent.nom,t[1],t[2]]
...
class VueSysteme(Perspective):
    ...
    def afficherpartie(self,mod): 
        ...
        for p in self.systeme.planetes:
            x,y=hlp.getAngledPoint(math.radians(p.angle),p.distance*self.UA2pixel,xl,yl)
            n=p.taille*self.UA2pixel
            self.canevas.create_oval(x-n,y-n,x+n,y+n,fill="red",tags=(self.systeme.proprietaire,"planete",p.id,"inconnu",self.systeme.id,int(x),int(y)))
            x,y=hlp.getAngledPoint(math.radians(p.angle),p.distance*UAmini,100,100)
            self.minimap.create_oval(x-mini,y-mini,x+mini,y+mini,fill="red",tags=("planete"))
    ...
    def __init__(self,parent,syste,plane):
        ...
        for i in range(self.planete.terrainTailleCarre):
            for j in range(self.planete.terrainTailleCarre):
                self.canevas.create_rectangle(i*tailleTile, j*tailleTile, i*tailleTile + tailleTile, j*tailleTile + tailleTile, fill=self.planete.terrainColor[i][j], outline = "")
    ...
    def affichermodelestatique(self,s,p):
        ...
        for i in range(p.terrainTailleCarre):
            for j in range(p.terrainTailleCarre):
                self.minimap.create_rectangle(i*t, j*t, i*t + t, j*t + t, fill = p.terrainColor[i][j], outline = "");
