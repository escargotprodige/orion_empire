        ...
        if systemeplanetaire:
            ...
            for i in range(nbplanetes):
                ...
                while (distanceBool):
                    ...
                    while (distsol < 1):
                        distsol=random.randrange(250)/10 #distance en unite astronomique 150000000km # Distance entre planete et soleil
                    angle=random.randrange(360) # angle de depart
                     
                     
                    if len(self.planetes) > 1: #and distsol > 0.5 : # Faut aumoins avoir une planete avant de checker et que la planete sois pas sur la soleil
                        for i in range(len(self.planetes)):
                            distanceBool = False
                            if distsol <= self.planetes[i].distance - .2 and distsol >= self.planetes[i].distance + .2 : # Si fonctionne pas relancer la position
                                distanceBool = True
